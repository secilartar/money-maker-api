...
def analiz_et(hisse: str = Query("BRSAN"), x_api_key: str = Header(None)):
    ...
    5. **ÇOK ÖNEMLİ KURAL:**
        - Asla Markdown tablosu, | işareti, tablo satırı veya karmaşık tablo formatı KULLANMA.
        - Sadece aşağıdaki gibi temiz, alt alta madde işaretli liste kullan.
        - Her satır mutlaka tire (-) ile başlamalı.
        - Formatı birebir kopyala, ekstra sütun veya satır ekleme:
        - Aşağıdaki formatı birebir takip ederek temiz, alt alta maddeler halinde "SEVİYE VE HEDEF LİSTESİ" sun:
